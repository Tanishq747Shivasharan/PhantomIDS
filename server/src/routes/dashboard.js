...
router.get('/api/attacks', requireAuth, (req, res) => {
  ...
  try {
    ...
    const rows  = db.prepare(`SELECT * FROM attack_log WHERE ${where} ORDER BY timestamp DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
    const total = db.prepare(`SELECT COUNT(*) as total FROM attack_log WHERE ${where}`).get(...params).total;
